...
<form action="/lantaisetpintuop/tambah" method="post">
    {{ csrf_field()}}
  <div class="isi">
      <h2>Choose Slot To Add Door</h2>        
     
      <!--Cetak Slot Lantai -->
          @foreach($slot as $sub)
              
          @if($sub->Status_Slot == 0)
          <label class="kotak11"><input type="radio" name="knk" value="{{$sub->id}}" class="input1" required>
          <div class="abed"></div>
          </label>
          ...
          <div class="qma">
            ...
            <select name="status_slot">
                <option value="5">Pintu Masuk</option>
                <option value="6">Pintu Keluar</option>
            </select>
              <input type="hidden" name="lid" value="{{$l->id}}">
              <input type="hidden" name="jl" value="{{$l->Jumlah_Slot}}">
            </br></br>
            Pintu Masuk<img src="/images/pintumasuk.png" width="50px" height="50px">   
            </br></br>
            Pintu Keluar<img src="/images/pintukeluar.png" width="50px" height="50px">   
            </br></br>
              <input type="submit" value="Tambah Pintu" class="btn btn-primary">         
              @endforeach
          </div> 
          </div>
